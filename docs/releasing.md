...
4. **Smithery** (`publish.yml`, `smithery` job, `needs: publish`) — waits for npm to serve the new version, builds the cross-platform MCPB bundle (`pnpm run build:mcpb` / `scripts/build-mcpb.mjs`, which vendors the *published* npm package), attaches `qring-<version>.mcpb` to the GitHub release, publishes it to `i4ctime/q-ring` through the registry API with `scripts/smithery-publish.mjs` (`SMITHERY_API_KEY` repo secret) — not the `smithery` CLI, which cannot send tool schemas for bundles (smithery-ai/cli#787) and leaves the listing at "No capabilities found"; the build script captures the server's live `tools/list` into `dist-mcpb/qring-<version>.smithery.json` and the publish script sends it as the release's server card — and best-effort re-applies the listing metadata + icon (bundle publishes have reset them before). Downstream of the npm/MCP publish on purpose — a Smithery failure never blocks the critical path.
